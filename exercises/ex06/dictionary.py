...
def invert(values_to_invert: dict[str, str]) -> dict[str, str]:
    ...
    for key, value in values_to_invert.items():
        if value in dictionary_after_inversion:
            raise KeyError(f"the key {dictionary_after_inversion[value]} has already been encountered. Associate this value with a new, unique key.")
        else:
